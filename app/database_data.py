...
from .models import (CPU, GPU, RAM, Assembly, AssemblyComponentAssociation,
                     Brand, MemoryType, Motherboard, SocketType, Soundcard)
    ...
    with session_factory() as session:
        try:
            ...
            brand_data = [
                ...
                ("INTEL", "cpu"), ("AMD", "cpu"),
                ("NVIDIA", "gpu"), ("ZOTAC", "gpu"),
                ("CORSAIR", "ram"), ("KINGSTON", "ram"),
                ("CREATIVE", "soundcard"), ("BEHRINGER", "soundcard"),
            ]
            ...
            def get_brand_id(brand_name):
                return session.query(Brand).filter_by(name=brand_name).first().id

            def get_socket_id(socket_name):
                return session.query(SocketType).filter_by(name=socket_name).first().id

            def get_memory_id(memory_name):
                return session.query(MemoryType).filter_by(name=memory_name).first().id
