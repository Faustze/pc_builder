...
from .forms import (AssemblySelectForm, BrandForm, CPUForm, GPUForm,
                    MemoryTypeForm, MotherboardForm, RAMForm, SocketForm,
                    SoundcardForm, get_memory_type_choices)
from .models import (CPU, GPU, RAM, Assembly, AssemblyComponentAssociation,
                     Brand, Component, MemoryType, Motherboard, SocketType,
                     Soundcard)
...
def populate_component_choices(form, session, selected_ids=None):
    ...
    mb_choices = [
        (mb.id, f"{mb.brand_rel.name} {mb.model} | Socket: {mb.socket_type_rel.name} | RAM: {mb.memory_type_rel.name}")
    ...
    cpu_choices = [
        (cpu.id, f"{cpu.brand_rel.name} {cpu.model} | Socket: {cpu.socket_type_rel.name} | "
    ...
    ram_choices = [
        (ram.id, f"{ram.brand_rel.name} {ram.model} | {ram.memory_type_rel.name} | "
...
def populate_assembly_components(session, assembly, selected_components):
    ...
    for component in components:
        ...
        assoc = AssemblyComponentAssociation(
            component=component,
            quantity=quantity
        )
...
def init_routes(app):
    ...
    def get_components_page():
        ...
        component_entities = with_polymorphic(
            Component,
            [Motherboard, CPU, GPU, RAM, Soundcard]
        )
        ...
        return render_template("components/components.html",
    ...
    def add_component_motherboard():
        ...
        with session_factory() as session:
            ...
            socket_types = (
                session.query(SocketType)
                .all()
            )
            memory_types = (
                session.query(MemoryType)
                .all()
            )
        ...
        if form.validate_on_submit():
            try:
                ...
                if 'unique constraint' in str(e.orig).lower():
                    flash(f'Error: Component with model "{form.model.data}" already exists.', 'danger')
                else:
        ...
        return render_template('components/add_component_motherboard.html',
                               form=form)
    ...
    def add_component_cpu():
        ...
        return render_template('components/add_component_cpu.html',
                               form=form)
    ...
    def add_component_gpu():
        ...
        return render_template('components/add_component_gpu.html',
                               form=form)
    ...
    def add_component_ram():
        ...
        return render_template('components/add_component_ram.html',
                               form=form)
    ...
    def add_component_soundcard():
        ...
        return render_template('components/add_component_soundcard.html',
                               form=form)
    ...
    def edit_component(component_id):
        ...
        with session_factory() as session:
            ...
            if component.component_type == 'motherboard':
                ...
                socket_types = (
                    session.query(SocketType)
                    .order_by(SocketType.name)
                    .all()
                )
                memory_types = (
                    session.query(MemoryType)
                    .order_by(MemoryType.name)
                    .all()
                )
        ...
        try:
            with session_factory() as session:
                ...
                if component.component_type == 'motherboard':
                    session.execute(
                        delete(Motherboard)
                        .where(Motherboard.id is component.id)
                    )
                elif component.component_type == 'cpu':
                    session.execute(
                        delete(CPU)
                        .where(CPU.id is component.id)
                    )
                elif component.component_type == 'ram':
                    session.execute(
                        delete(RAM)
                        .where(RAM.id is component.id)
                    )
                elif component.component_type == 'gpu':
                    session.execute(
                        delete(GPU)
                        .where(GPU.id is component.id)
                    )
                elif component.component_type == 'soundcard':
                    session.execute(
                        delete(Soundcard)
                        .where(Soundcard.id is component.id)
                    )
    ...
    def get_classificators_page():
        ...
        return render_template("classificators/classificators.html",
    ...
    def get_assemblies_page():
        with session_factory() as session:
            ...
            return render_template("assemblies/assemblies.html",
    ...
    def get_assembly(assembly_id):
        with session_factory() as session:
            ...
            return render_template("assemblies/assembly_detail.html",
                                   assembly=assembly,
    ...
    def edit_assembly(assembly_id):
        ...
        with session_factory() as session:
            ...
            selected_ids = {
                'motherboard_id': comp_map.get('motherboard').id if comp_map.get('motherboard') else None,
                'cpu_id': comp_map.get('cpu').id if comp_map.get('cpu') else None,
                'gpu_id': comp_map.get('gpu').id if comp_map.get('gpu') else None,
                'ram_id': comp_map.get('ram').id if comp_map.get('ram') else None,
                'soundcard_id': comp_map.get('soundcard').id if comp_map.get('soundcard') else None
            }
            ...
            if request.method == 'GET':
                ...
                form.motherboard_id.data = comp_map.get('motherboard').id if comp_map.get('motherboard') else None
                form.cpu_id.data = comp_map.get('cpu').id if comp_map.get('cpu') else None
                form.gpu_id.data = comp_map.get('gpu').id if comp_map.get('gpu') else None
                form.ram_id.data = comp_map.get('ram').id if comp_map.get('ram') else None
                form.soundcard_id.data = comp_map.get('soundcard').id if comp_map.get('soundcard') else None

                form.gpu_is_integrated.data = 'gpu' not in comp_map

                assoc_map = {assoc.component.component_type: assoc for assoc in assembly.components_association}

                form.motherboard_quantity.data = assoc_map.get('motherboard').quantity if assoc_map.get(
                    'motherboard') else 1
                form.cpu_quantity.data = assoc_map.get('cpu').quantity if assoc_map.get('cpu') else 1
                form.gpu_quantity.data = assoc_map.get('gpu').quantity if assoc_map.get('gpu') else 1
                form.ram_quantity.data = assoc_map.get('ram').quantity if assoc_map.get('ram') else 1
                form.soundcard_quantity.data = assoc_map.get('soundcard').quantity if assoc_map.get('soundcard') else 1
                ...
                try:
                    assembly.name = form.assembly_name.data
                    assembly.quantity = form.assembly_quantity.data
    ...
    def download_assembly_xml(assembly_id):
        with session_factory() as session:
            ...
            for component in assembly.components:
                comp_el = Et.SubElement(root, "component", attrib={
